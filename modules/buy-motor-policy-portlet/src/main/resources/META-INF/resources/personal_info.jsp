...
{
	if($(".driversTbl tr").size()==1 && !<%=isMainDriver%>)
		return false;
	objToRemove = obj;
...
// 		}
	...
	{
		var currDriversCount = $(".driversTbl tr").size();
		...
		$.when( $(".driversTbl tr").size() == currDriversCount+1).then(function(){
			manageDriversChange();
		});
	}
...
// 	}
	if($('.driversTbl').find("input[name='<portlet:namespace />quotDrivers']").size() >= max_fields)
	{
	    $("#openDriverPopup").hide();
	}
}
...
function manageDriversChange()
{
	...
	if($(".driversTbl tr.error-driver").size()>0 ||((underAgeDriver || !<%=isMainDriver%>) && ($(".driversTbl tr").size() == 0 )))
		$("#submitBtn").attr("disabled",true);
	else
		$("#submitBtn").removeAttr("disabled");
	
	if($(".driversTbl tr").size() == 0)
		$(".drv_stat").show();
	else
		$(".drv_stat").hide();
}
