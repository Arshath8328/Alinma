...
import com.atmc.mob.acl.ws.utils.SOAPUtils;
...
//		Crypto crypto = CryptoFactory.getInstance(CRYPTO_PROPS);
		PasswordEncryptor passwordEncryptor = new JasyptPasswordEncryptor("apache_pwd");
		Properties properties = CryptoFactory.getProperties(CRYPTO_PROPS,Loader.getClassLoader(CryptoFactory.class));
		Crypto crypto = CryptoFactory.getInstance(properties, Loader.getClassLoader(CryptoFactory.class), passwordEncryptor);
