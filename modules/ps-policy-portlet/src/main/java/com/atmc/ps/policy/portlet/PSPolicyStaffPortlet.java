...
public class PSPolicyStaffPortlet extends MVCPortlet {
	...
	static final String		PROPORSAL_NO_STRING		= "proposalNo";
	static final String		FROM_DATE				= "fromDate";
	static final String		TO_DATE					= "toDate";
	static final String		LEGAL_HEIRS				= "Legal Heirs";
	...
	public void render(RenderRequest renderRequest, RenderResponse renderResponse) throws IOException, PortletException {
		...
		String proposalNo = ParamUtil.getString(renderRequest, PROPORSAL_NO_STRING);
		if (!Validator.isBlank(proposalNo)) {
			try {
				...
				renderRequest.getPortletSession().removeAttribute(PROPORSAL_NO_STRING);
		...
		if ("details".equals(cmd)) {
			try {
				String fromDate = ParamUtil.getString(renderRequest, FROM_DATE);
				String toDate = ParamUtil.getString(renderRequest, TO_DATE);
				...
				soaDetail.getPsCoverageDetails().stream().forEach(cd -> cd.setBenefit_name(
						(englishArabicNamesMap.get(cd.getBenefit_name().toLowerCase())) == null ? cd.getBenefit_name() : englishArabicNamesMap.get(cd.getBenefit_name().toLowerCase())));
				...
				soaDetail.setInvestment_strategy(englishArabicNamesMap.get(soaDetail.getInvestment_strategy().toLowerCase()) == null ? soaDetail.getInvestment_strategy()
						: englishArabicNamesMap.get(soaDetail.getInvestment_strategy().toLowerCase()));
				...
				soaDetail.getPsBeneficiaryDetails().stream().forEach(bd -> {
					if (bd.getRelationship() == null) {
						bd.setBeneficiary_name(englishArabicNamesMap.get(LEGAL_HEIRS));
						bd.setRelationship("");
					} else {
				...
				renderRequest.setAttribute(FROM_DATE, dateFormat.format(date1));
				renderRequest.setAttribute(TO_DATE, dateFormat.format(date2));
	...
	public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws IOException, PortletException {
		try {
			...
			String proposalNo = ParamUtil.getString(resourceRequest, PROPORSAL_NO_STRING);
			...
			soaDetail.setFrequency(englishArabicNamesMap.get(soaDetail.getFrequency()));
			soaDetail.setProduct_desc(englishArabicNamesMap.get(soaDetail.getProduct_desc().toLowerCase()));
			soaDetail.getPsCoverageDetails().stream().forEach(cd -> cd
					.setBenefit_name((englishArabicNamesMap.get(cd.getBenefit_name().toLowerCase())) == null ? cd.getBenefit_name() : englishArabicNamesMap.get(cd.getBenefit_name().toLowerCase())));
			soaDetail.getPsCoverageDetails().stream().forEach(cd -> cd.setBenefit_type(englishArabicNamesMap.get(cd.getBenefit_type().toLowerCase())));
			soaDetail.setInvestment_strategy(englishArabicNamesMap.get(soaDetail.getInvestment_strategy().toLowerCase()) == null ? soaDetail.getInvestment_strategy()
					: englishArabicNamesMap.get(soaDetail.getInvestment_strategy().toLowerCase()));
			soaDetail.setCurrency(englishArabicNamesMap.get(soaDetail.getCurrency().toLowerCase()));
	...
	public Map<String, String> setArabicText() {
		...
		map.put(LEGAL_HEIRS, LanguageUtil.get(resourceBundle, legalHeirs));
		map.put("Legal heirs", LanguageUtil.get(resourceBundle, legalHeirs));
		map.put("Legal hiers", LanguageUtil.get(resourceBundle, legalHeirs));
