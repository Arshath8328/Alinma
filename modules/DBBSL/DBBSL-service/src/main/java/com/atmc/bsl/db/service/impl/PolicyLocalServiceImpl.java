...
import com.atmc.bsl.db.domain.policy.*;
 ...
 *
 * <p>
 * All custom service methods should be put in this class. Whenever methods are added, rerun ServiceBuilder to copy their definitions into the <code>com.atmc.bsl.db.service.PolicyLocalService</code> interface.
 *
 * <p>
 ...
 */
@Component(
	property = "model.class.name=com.atmc.bsl.db.model.Policy",
...
public class PolicyLocalServiceImpl extends PolicyLocalServiceBaseImpl {
	 ...
	 * com.ejada.atmc.bsl.db.service.PolicyLocalServiceUtil} to access the policy local service.
	 */
	...
	private List<Policy> getCustomPoliciesForPolicyList(List<PolicyHDR> policyHdrList) throws PortalException
	{
		...
		if(policyHdrList != null && !policyHdrList.isEmpty())
		{
			for(PolicyHDR policyHdr : policyHdrList)
			{
				...
				else if(product.equals("430")||product.equals("450")||product.equals("470")||product.equals("480")||product.equals("490"))
				{
	...
	public PolicyVehicle getCustomVeh(String policyNo, String vehId)  throws PortalException{
		...
		//_log.debug("performance test: after getting Vehicle Drivers, Vehicle Drivers size"+PolicyVehDriversList.size() );
		...
		//_log.debug("performance test: after getting Vehicle Covers, Vehicle Covers size"+PolicyVehCoversList.size() );
		...
		if(policyHdrList != null && !policyHdrList.isEmpty())
		{
			for(PolicyHDR policyHdr : policyHdrList)
			{
				...
				if(product.equals("13101")||product.equals("13102"))
				{
					...
					List<PolicyVEH> PolicyVehList = policyHdr.getPolicyVEHs();
					//_log.debug("performance test: after getting Vehicle List, Vehicle List size"+PolicyVehList.size() );
				...
				else if(product.equals("SMEE")||product.equals("CCHA"))
				{
					...
					List<PolicyMemberDetails> policyMembersList = policyHdr.getPolicyMembers();
					// _log.debug("performance test: after getting policyMembersList, policyMembersList size"+policyMembersList.size() );
					if(policyMembersList != null & !policyMembersList.isEmpty())
						for(PolicyMemberDetails pMember : policyMembersList)
						{
							PolicyMember customMem = new PolicyMember();
				...
				else if(product.equals("430")||product.equals("450")||product.equals("470")||product.equals("480")||product.equals("490"))
				{
					...
					//_log.debug("performance test: after getting policyPayments, policyPayments size"+policyPayments.size() );
					...
					// _log.debug("performance test: after getting policyRidersList, policyRidersList size"+policyRidersList.size() );
					...
					// _log.debug("performance test: after getting policyFundsList, policyFundsList size"+policyFundsList.size() );
					...
					// _log.debug("performance test: after getting policyBenfsList, policyBenfsList size"+policyBenfsList.size() );
	...
	public List<HashMap<String, Object>> getPolicySummary(String iqamaId) throws PortalException
	{
		...
		for(PolicyHDR policyData: PolicyHDRList)
		{
			...
			else if(policyData.getProduct().equals("430") || policyData.getProduct().equals("450")||policyData.getProduct().equals("470") || policyData.getProduct().equals("480")||policyData.getProduct().equals("490"))
			{
	...
	public Policy getVehiclePolicyData(String plateNo, String plateL1, String plateL2, String plateL3, String language) {
		Policy customPolicy = null;
		PolicyHDR policyHdr;
		try {
