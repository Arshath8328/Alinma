...
public class Encryptor {
	...
	public static final String KEY_ALGORITHM = StringUtil
			.toUpperCase(GetterUtil.getString(PropsUtil.get(PropsKeys.COMPANY_ENCRYPTION_ALGORITHM)));

	public static final int KEY_SIZE = GetterUtil.getInteger(PropsUtil.get(PropsKeys.COMPANY_ENCRYPTION_KEY_SIZE));

	public static final String PROVIDER_CLASS = GetterUtil.getString(
			SystemProperties.get(Encryptor.class.getName() + ".provider.class"), Encryptor.SUN_PROVIDER_CLASS);
	...
	public static String decryptUnencodedAsString(Key key, byte[] encryptedBytes) throws EncryptorException {
		...
		try {
			byte[] decryptedBytes = decryptUnencodedAsBytes(key, encryptedBytes);
		...
		try {
			...
			if (ServerDetector.isWebSphere() && PROVIDER_CLASS.equals(SUN_PROVIDER_CLASS)) {
				...
				if (_log.isInfoEnabled()) {
					_log.info("WebSphere does not have " + SUN_PROVIDER_CLASS + ", using " + IBM_PROVIDER_CLASS
							+ " instead");
				}
				...
				if (_log.isInfoEnabled()) {
					_log.info("IBM JVM does not have " + SUN_PROVIDER_CLASS + ", using " + IBM_PROVIDER_CLASS
							+ " instead");
				}
	...
	private static final Map<String, Cipher> _decryptCipherMap = new ConcurrentHashMap<>(1, 1F, 1);
	private static final Map<String, Cipher> _encryptCipherMap = new ConcurrentHashMap<>(1, 1F, 1);
