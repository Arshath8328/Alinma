...
public class TOTP {
	...
	public static int generateTOTP(byte[] key, long time, int digits, String crypto) {
		...
		if (String.valueOf(otp).length() < 4) {
			...
			String x = String.valueOf(ran.nextInt(9));
			String er = otp + x;
			otp = Integer.parseInt(er);
		}

		return Validator.isNotNull(PropsUtil.get("static.otp")) ? Integer.parseInt(PropsUtil.get("static.otp")) : otp;
	}
}
