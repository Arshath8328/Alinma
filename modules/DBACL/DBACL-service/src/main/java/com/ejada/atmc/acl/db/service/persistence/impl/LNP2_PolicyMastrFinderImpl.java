...
public class LNP2_PolicyMastrFinderImpl extends LNP2_PolicyMastrFinderBaseImpl implements LNP2_PolicyMastrFinder {
	...
	private final String	FIND_PS_POLICY_DETAILS_BY_ID				= "select lnp1.np1_proposal,         sysdate Statement_Date,         (select lnph.nph_fullname            from lnph_pholder@ILAS lnph           where lnph.nph_code = lnu1.nph_code) Policy_Holder,         (select lnad.nad_email1            from ilas_proposal_address lnad           where lnad.np1_proposal = lnp1.np1_proposal) Email_Address,         (select lppr.ppr_descr            from lppr_product@ILAS lppr           where lppr.ppr_prodcd = lnpr.ppr_prodcd) Product_Desc,         (select lnad.nad_mobile            from ilas_proposal_address lnad           where lnad.np1_proposal = lnp1.np1_proposal) Contact_No,         (select lnad.homeaddress            from ilas_proposal_address lnad           where lnad.np1_proposal = lnp1.np1_proposal) homeaddress,         (select lnad.mailing_address            from ilas_proposal_address lnad           where lnad.np1_proposal = lnp1.np1_proposal) mailing_address,         (select lcst.cst_descr            from lcst_ppstatus@ILAS lcst           where lcst.cst_statuscd = lnp1.cst_statuscd) Status_Descr,         lnp2.np2_commendate Inception_Date,         nvl(lnp2.np2_totpremium, 0) + nvl(lnp2.np2_totload, 0) Installment_Amount,         lnpr.npr_premiumter Plan_Duration,         (select sum(d.dueamount)            from view_adjusted_premium@ilas d           where d.np1_proposal = lnp1.np1_proposal             and d.oal_initiatedate between                 to_date('01-01-2015', 'DD-MM-YYYY') and ?) Total_Amount_Received,         lnp2.cmo_mode Frequency    from lnp1_policymastr@ILAS lnp1,         lnp2_policymastr@ILAS lnp2,         lnpr_product@ILAS     lnpr,         lnu1_underwriti@ILAS  lnu1  where lnp1.np1_proposal = lnp2.np1_proposal     and lnp2.np2_setno = (select max(x.np2_setno) from lnp2_policymastr@ILAS x where x.np1_proposal = lnp2.np1_proposal and x.np2_approved = 'Y')     and lnp2.np1_proposal = lnpr.np1_proposal     and lnp2.np2_setno = lnpr.np2_setno     and lnp2.np1_proposal = lnu1.np1_proposal     and lnu1.nu1_life = 'F'     and lnpr.npr_basicflag = 'Y'     and lnp1.np1_proposal = ?  ";
	...
	private final String	FIND_SINGLE_ENTRY_FIELDS					= "select lnp1.np1_proposal,         uptodt statement_date,         (select lnph.nph_fullname            from lnph_pholder@ilas lnph           where lnph.nph_code = lnu1.nph_code) policy_holder,         (select lnad.nad_email1            from ilas_proposal_address lnad           where lnad.np1_proposal = lnp1.np1_proposal) email_address,         (select lppr.ppr_descr            from lppr_product@ilas lppr           where lppr.ppr_prodcd = lnpr.ppr_prodcd) product_desc,         (select lnad.nad_mobile            from ilas_proposal_address lnad           where lnad.np1_proposal = lnp1.np1_proposal) contact_no,         (select lnad.homeaddress            from ilas_proposal_address lnad           where lnad.np1_proposal = lnp1.np1_proposal) homeaddress,         (select lnad.mailing_address            from ilas_proposal_address lnad           where lnad.np1_proposal = lnp1.np1_proposal) mailing_address,         (select lcst.cst_descr            from lcst_ppstatus@ilas lcst           where lcst.cst_statuscd = lnp1.cst_statuscd) status_descr,         lnp2.np2_commendate inception_date,         uptodt processing_dt,         lnpr.npr_premiumter plan_duration,         lnp2.cmo_mode frequency,         (select max(lc.cbd_valuedate)            from lnfu_funds@ilas f, lcbd_bidoffer@ilas lc           where f.cfu_fundcode = lc.cfu_fundcode             and f.np1_proposal = lnp1.np1_proposal             and f.np2_setno = lnp2.np2_setno             and nvl(f.nfu_rate, 0) <> 0             and lc.cbd_valuedate <= uptodt) investment_date,         nvl(lnp2.np2_totpremium, 0) + nvl(lnp2.np2_totload, 0) installment_amount,         (select sum(d.dueamount)            from view_adjusted_premium@ilas d           where d.np1_proposal = lnp1.np1_proposal             and d.oal_initiatedate between to_date('01-01-2015', 'DD-MM-YYYY') and uptodt) total_amount_received,         round((case                 when lnp2.cmo_mode = 'M' then (floor(months_between(uptodt, lnp2.np2_commendate))+1)                 when lnp2.cmo_mode = 'A' then (floor(months_between(uptodt, lnp2.np2_commendate)/12)+1)                 when lnp2.cmo_mode = 'S' then 1                 when lnp2.cmo_mode = 'Q' then (floor(months_between(uptodt, lnp2.np2_commendate)/3)+1)                 when lnp2.cmo_mode = 'H' then (floor(months_between(uptodt, lnp2.np2_commendate)/6)+1)               end) *               (nvl(lnp2.np2_totpremium, 0) + nvl(lnp2.np2_totload, 0)),2) total_premium_due,         round((case                 when lnp2.cmo_mode = 'M' then (floor(months_between(uptodt, lnp2.np2_commendate))+1)                 when lnp2.cmo_mode = 'A' then (floor(months_between(uptodt, lnp2.np2_commendate)/12)+1)                 when lnp2.cmo_mode = 'S' then 1                 when lnp2.cmo_mode = 'Q' then (floor(months_between(uptodt, lnp2.np2_commendate)/3)+1)                 when lnp2.cmo_mode = 'H' then (floor(months_between(uptodt, lnp2.np2_commendate)/6)+1)               end) *               (nvl(lnp2.np2_totpremium, 0) + nvl(lnp2.np2_totload, 0)),2) -         (select sum(d.dueamount)            from view_adjusted_premium@ilas d           where d.np1_proposal = lnp1.np1_proposal             and d.oal_initiatedate between                 to_date('01-01-2015', 'DD-MM-YYYY') and uptodt) premium_not_paid,         nvl((select sum(d.dueamount)               from view_adjusted_premium@ilas d, last_unitdate b              where d.oal_initiatedate between                    to_date('01-01-2015', 'dd-mm-yyyy') and uptodt                and d.np1_proposal = lnp1.np1_proposal                and trunc(oal_initiatedate) <= pricedate),             0) premium_invested,         nvl((select sum(d.dueamount)               from view_adjusted_premium@ilas d, last_unitdate b              where d.oal_initiatedate between                    to_date('01-01-2015', 'dd-mm-yyyy') and uptodt                and d.np1_proposal = lnp1.np1_proposal                and trunc(oal_initiatedate) > pricedate),             0) premium_to_be_invested,         nvl(lnp1.np1_adhocpremium, 0) topup_premium,         nvl(lnp1.np1_creditbalance, 0) credit_balance,         nvl((select sum(lsbp.sbp_amount) withdrawal               from larq_request@ilas larq, lsbp_benefpayment@ilas lsbp              where lnp1.np1_proposal = lnp1.np1_proposal                and larq.arq_requestype = 'RSET'                and larq.cdc_code = '001'                and larq.np1_proposal = lnp1.np1_proposal                and larq.pfs_acntyear = lsbp.pfs_acntyear                and larq.arq_requestype = lsbp.arq_requestype                and larq.arq_requestno = lsbp.arq_requestno),             0) withdrawal,         round((case                 when lnp2.cmo_mode = 'M' then (floor(months_between(uptodt, lnp2.np2_commendate))+1)                 when lnp2.cmo_mode = 'A' then (floor(months_between(uptodt, lnp2.np2_commendate)/12)+1)                 when lnp2.cmo_mode = 'S' then 1                 when lnp2.cmo_mode = 'Q' then (floor(months_between(uptodt, lnp2.np2_commendate)/3)+1)                 when lnp2.cmo_mode = 'H' then (floor(months_between(uptodt, lnp2.np2_commendate)/6)+1)               end)) premium_due_duration,         (select max(instno)            from view_adjusted_premium@ilas d, policy_master@ilas p           where d.oal_initiatedate between to_date('01-01-2015', 'DD-MM-YYYY') and sysdate - 1             and d.np1_proposal = p.np1_proposal             and d.np1_proposal = lnp1.np1_proposal             and d.oal_initiatedate <= uptodt) installment_paid,         round((case                 when lnp2.cmo_mode = 'M' then (floor(months_between(uptodt, lnp2.np2_commendate))+1)                 when lnp2.cmo_mode = 'A' then (floor(months_between(uptodt, lnp2.np2_commendate)/12)+1)                 when lnp2.cmo_mode = 'S' then 1                 when lnp2.cmo_mode = 'Q' then (floor(months_between(uptodt, lnp2.np2_commendate)/3)+1)                 when lnp2.cmo_mode = 'H' then (floor(months_between(uptodt, lnp2.np2_commendate)/6)+1)               end)) -         (select max(instno)            from view_adjusted_premium@ilas d, policy_master@ilas p           where d.oal_initiatedate between                 to_date('01-01-2015', 'DD-MM-YYYY') and sysdate - 1             and d.np1_proposal = p.np1_proposal             and d.np1_proposal = lnp1.np1_proposal             and d.oal_initiatedate <= uptodt) installment_remaining    from lnp1_policymastr@ilas lnp1,         lnp2_policymastr@ilas lnp2,         lnpr_product@ilas     lnpr,         lnu1_underwriti@ilas  lnu1  where lnp1.np1_proposal = lnp2.np1_proposal     and lnp2.np2_setno = (select max(x.np2_setno)                             from lnp2_policymastr@ilas x                            where x.np1_proposal = lnp2.np1_proposal                              and x.np2_approved = 'Y')     and lnp2.np1_proposal = lnpr.np1_proposal     and lnp2.np2_setno = lnpr.np2_setno     and lnp2.np1_proposal = lnu1.np1_proposal     and lnu1.nu1_life = 'F'     and lnpr.npr_basicflag = 'Y'     and lnp1.np1_proposal = ?";
		...
		try {
			...
			if (Validator.isNotNull(data)) {
				for (Object[] objects : data) {
					...
					String emailAddress = (String) objects[i++];
					psPolicyDetail.setEmail_address(emailAddress != null ? emailAddress : "");
					psPolicyDetail.setProduct_desc((String) objects[i++]);
					String contactNo = (String) objects[i++];
					psPolicyDetail.setContact_no(contactNo != null ? contactNo : "");
					String nadAddress = (String) objects[i++];
					psPolicyDetail.setNad_address1(nadAddress != null ? nadAddress : "");
					String mailingAdddress = (String) objects[i++];
					psPolicyDetail.setMailingAddress1(mailingAdddress != null ? mailingAdddress : "");
...
//					psPolicyDetail.setMailingAddress3(soaDetail.getMailingAddress3() == null ? " " : soaDetail.getMailingAddress3());
	...
	public List<SOADetail> findPSPolicyStatementDetailsByIdAndDate(String id, String fromDate, String toDate) throws PortalException {
		...
		SOADetail psStatementDetail;

		psStatementDetail = findSingleEntryFundStatementDetails(id, toDate);

		List<PSFundDetail> psFundDetails = findPSPolicyFundDetailsById(id, toDate);
		...
		List<PSStatementAccount> psStatementAccounts = findStatementsOfAccount(id, toDate);
		...
		psStatementDetail.setStatement_date(toDate);

		if (!((psStatementDetail.getStatus_descr() != "ACTIVE  (INFORCE)") || (psStatementDetail.getStatus_descr() != "REINSTATED")) || (isPolicySurrendered)
				|| (psStatementDetail.getStatus_descr().contains("CANCELLED")))
		{
			psStatementDetail.setNo_units(0);
			psStatementDetail.setCurrent_unit_value(0);
			psStatementDetail.setTotal_saving_value(0);
	...
	private SOADetail findCustomerAddressByIqumaId(String id) throws PortalException {
		Session session = null;
		try {
			session = openSession();
			...
			if (Validator.isNotNull(data)) {
				for (Object[] objects : data) {
					...
					String poboxNo = (String) objects[i++];
					String mailingAddress = (String) objects[i++];
					String postalCode = (String) objects[i++];
					soaDetail.setMailingAddress1((mailingAddress != null ? mailingAddress : "") + " \n " + (poboxNo != null ? poboxNo : "") + " \n " + (postalCode != null ? postalCode : ""));
	...
	private List<PSCoverageDetail> findPSPolicyCoverageDetailsById(String id) throws PortalException {
		Session session = null;
		try {
			session = openSession();
	...
	private List<PSFundDetail> findPSPolicyFundDetailsById(String id, String toDate) throws PortalException {
		Session session = null;
		try {
			session = openSession();
			String termQuery = FIND_PS_POLICY_FUND_DETAILS_BY_ID.replaceAll("uptodt", "'" + toDate + "'");
	...
	private List<PSBeneficiaryDetail> findPSPolicyBeneficiaryDetailsById(String id) throws PortalException {
		Session session = null;
		try {
			session = openSession();
	...
	private List<PSStatementAccount> findStatementsOfAccount(String id, String toDate) {
		this.toDate = toDate;
		Session session = null;
		try {
			session = openSession();
			...
			if (Validator.isNotNull(data)) {
				for (Object[] objects : data) {
					...
					String currentStatus = (String) objects[i++];
					psStatementAccount.setCurrent_status(currentStatus != null ? currentStatus : "");
					...
					if (psStatementAccount.getPayment_status().equalsIgnoreCase("Not Paid"))
						psStatementAccount.setCurrent_status("");

					if (psStatementAccount.getInstallment_amount() < 0) {
						psStatementAccount.setPayment_status("Reversed");
						psStatementAccount.setCurrent_status("");
					}
	...
	private SOADetail findSingleEntryFundStatementDetails(String id, String toDate) {
		Session session = null;
		try {
			session = openSession();
			String tempQuery = FIND_SINGLE_ENTRY_FIELDS.replaceAll("uptodt", "'" + toDate + "'");
			...
			if (Validator.isNotNull(data)) {
				for (Object[] objects : data) {
					int i = 0;
					BigDecimal topUp;
					SOADetail psPolicyDetail = new SOADetail();
					...
					psPolicyDetail.setNad_address1(nadAddress != null ? nadAddress : "");
					String mailingAddress = (String) objects[i++];
					psPolicyDetail.setMailingAddress1(mailingAddress != null ? mailingAddress.replaceAll("~", "\n") : "");
					psPolicyDetail.setStatus_descr((String) objects[i++]);
					psPolicyDetail.setInception_date(dateFormat.format(new Date(((Timestamp) objects[i++]).getTime())));
					i++; // processing_dt

					psPolicyDetail.setPlan_duration(String.format("%.0f", Math.abs(Math.abs(((BigDecimal) objects[i++]).doubleValue()))));
					psPolicyDetail.setFrequency((String) objects[i++]);
					psPolicyDetail.setInvestementDate(dateFormat.format(new Date(((Timestamp) objects[i++]).getTime())));
					psPolicyDetail.setGeneral_installment_amount(String.format("%.0f", Math.abs(Math.abs(((BigDecimal) objects[i++]).doubleValue()))));
					psPolicyDetail.setTotal_amount_received(String.format("%.0f", Math.abs(Math.abs(((BigDecimal) objects[i++]).doubleValue()))));
					psPolicyDetail.setPremium_paid(psPolicyDetail.getTotal_amount_received());

					psPolicyDetail.setTotal_premium_due(String.format("%.0f", Math.abs(((BigDecimal) objects[i++]).doubleValue())));
					BigDecimal premiumNotPaid = (BigDecimal) objects[i++];
					psPolicyDetail.setPremium_not_paid(String.valueOf(premiumNotPaid.doubleValue() < 0 ? 0 : premiumNotPaid.intValue()));
					psPolicyDetail.setPeremium_invested(String.format("%.0f", Math.abs(((BigDecimal) objects[i++]).doubleValue())));
					premiumToBeinvested = Math.abs(((BigDecimal) objects[i++]).doubleValue());
					psPolicyDetail.setPremium_to_be_invested(premiumToBeinvested.toString());

					topUp = (BigDecimal) objects[i++];
					psPolicyDetail.setTopup_premium(Validator.isNotNull(topUp) ? String.format("%.0f", (topUp).doubleValue()) : "0");
					psPolicyDetail.setCredit_balance(String.format("%.0f", Math.abs(((BigDecimal) objects[i++]).doubleValue())));
					psPolicyDetail.setWithdraw(String.format("%.0f", Math.abs(((BigDecimal) objects[i++]).doubleValue())));
					i++; // premium_due_duration

					psPolicyDetail.setInstallment_paid(String.valueOf(Math.abs(((BigDecimal) objects[i++]).intValue())));
					int installmentsRemaining = ((BigDecimal) objects[i++]).intValue();
					psPolicyDetail.setInstallment_remaining(String.valueOf(installmentsRemaining < 0 ? 0 : installmentsRemaining));

					soaDetails.add(psPolicyDetail);
				}
			}
			if (soaDetails.size() > 0)
				return soaDetails.get(0);
...
//			String withdraw = findWithdrawel(id);
//			psStatementDetail.setWithdraw(withdraw == null ? "0" : withdraw);
//
//			List<PSCoverageDetail> psCoverageDetails = findPSPolicyCoverageDetailsById(id);
//			psStatementDetail.setPsCoverageDetails(psCoverageDetails);
//
